...
spec = before (Text.writeFile "sample1.csv" sample1CSV ) $ describe "Application" $ do
  describe "lit un fichier CSV et produit un fichier ledger" $ do
    ...
    it "parse le fichier CSV" $ do
      ...
      parsed `shouldBe` [ Entry (fromJust $ isoDate "2018-05-14") "612000:Fournisseur-KPMG" "Frais tenu de comptes" Debit (12000) ]

    it "répartit une Entry équitablement quand il génère une transaction" $
      generateTransaction ["801000:Arnaud", "802000:Fred" ] (Entry (fromJust $ isoDate "2018-05-14") "612000:Fournisseur-KPMG" "Frais tenu de comptes" Debit (12000))
      `shouldBe`
    ...
    it "transforme une liste d'Entry en ledger" $ do
      generateLedger "sample2.ledger" [ "801000:Arnaud", "802000:Fred" ] [ Entry (fromJust $ isoDate "2018-05-14") "612000:Fournisseur-KPMG" "Frais tenu de comptes" Debit (12000) ]
