...
spec :: Spec
spec = before (Text.writeFile "sample1.csv" sample1CSV) $
    describe "Application" $
        describe "lit un fichier CSV et produit un fichier ledger" $ do
            ...
            it "répartit une Entry équitablement quand il génère une transaction" $
                generateTransaction
                    ( Entry
                        (fromJust $ isoDate "2018-05-14")
                        "612000:Fournisseur-KPMG"
                        "Frais tenu de comptes"
                        Debit
                        12000
            ...
            it "formate une transaction au format ledger" $
                render
                    ( Transaction
                        (fromJust $ isoDate "2018-05-14")
                        "Frais tenu de comptes"
